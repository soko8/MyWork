...
class CWindowWithoutCloseButton : public CWndContainer {
private:
	//--- dependent controls
...
protected:
	...
	void setBackgroundColor(const color bgColor) {
	   m_client_area.ColorBackground(bgColor);
	}
...
EVENT_MAP_BEGIN (CWindowWithoutCloseButton)
...
CWindowWithoutCloseButton::CWindowWithoutCloseButton(void) : m_panel_flag(false), m_minimized(false) {}

CWindowWithoutCloseButton::~CWindowWithoutCloseButton(void) {}


bool CWindowWithoutCloseButton::Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2) {
...
bool CWindowWithoutCloseButton::Save(const int file_handle) {
//--- check
...
//| Load                                                             |
//+------------------------------------------------------------------+
bool CWindowWithoutCloseButton::Load(const int file_handle) {
	if (file_handle == INVALID_HANDLE)
		return (false);
//--- load
...
//| Create "white border"                                            |
//+------------------------------------------------------------------+
bool CWindowWithoutCloseButton::CreateWhiteBorder(void) {
//--- coordinates
	int x1 = 0;
	int y1 = 0;
	int x2 = Width();
...
//| Create background                                                |
//+------------------------------------------------------------------+
bool CWindowWithoutCloseButton::CreateBackground(void) {
...
//--- create
	...
	if (!m_background.ColorBackground(CONTROLS_DIALOG_COLOR_BG))
		return (false);
	color border =
			(m_panel_flag) ? CONTROLS_DIALOG_COLOR_BG : CONTROLS_DIALOG_COLOR_BORDER_DARK;
...
//| Create window title                                              |
//+------------------------------------------------------------------+
bool CWindowWithoutCloseButton::CreateCaption(void) {
...
bool CWindowWithoutCloseButton::CreateClientArea(void) {
	int off = (m_panel_flag) ? 0 : 2 * CONTROLS_BORDER_WIDTH;
//--- coordinates
...
//--- create
	...
	if (!m_client_area.ColorBorder(CONTROLS_DIALOG_COLOR_CLIENT_BORDER))
		return (false);
	//m_client_area.VScrolled(true);
	//m_client_area.HScrolled(true);
...
//+------------------------------------------------------------------+
void CWindowWithoutCloseButton::OnClickCaption(void) {
}
//+------------------------------------------------------------------+
//| Start dragging the dialog box                                    |
//+------------------------------------------------------------------+
bool CWindowWithoutCloseButton::OnDialogDragStart(void) {
...
//--- constraints
	...
	m_drag_object.Limits(-CONTROLS_DRAG_SPACING,
                        -CONTROLS_DRAG_SPACING,
...
//+------------------------------------------------------------------+
bool CWindowWithoutCloseButton::OnDialogDragProcess(void) {
//--- checking
	if (m_drag_object == NULL)
		return (false);
//--- calculate coordinates
...
//+------------------------------------------------------------------+
bool CWindowWithoutCloseButton::OnDialogDragEnd(void) {
	if (m_drag_object != NULL) {
...
//+------------------------------------------------------------------+
//| Class CAppWindowWithoutCloseButton                                                 |
//+------------------------------------------------------------------+
class CAppWindowWithoutCloseButton: public CWindowWithoutCloseButton {
private:
	//--- dependent controls
...
public:
	CAppWindowWithoutCloseButton(void);
	~CAppWindowWithoutCloseButton(void);
...
//+------------------------------------------------------------------+
EVENT_MAP_BEGIN (CAppWindowWithoutCloseButton)
   ON_EVENT(ON_CLICK, m_button_minmax, OnClickButtonMinMax)
   ON_EXTERNAL_EVENT(ON_APP_CLOSE, OnAnotherApplicationClose)
EVENT_MAP_END(CWindowWithoutCloseButton)
//+------------------------------------------------------------------+
//| Constructor                                                      |
//+------------------------------------------------------------------+
CAppWindowWithoutCloseButton::CAppWindowWithoutCloseButton(void) :   m_program_type(WRONG_VALUE),
...
//| Destructor                                                       |
//+------------------------------------------------------------------+
CAppWindowWithoutCloseButton::~CAppWindowWithoutCloseButton(void) {
}
//+------------------------------------------------------------------+
//| Application dialog initialization function                       |
//+------------------------------------------------------------------+
bool CAppWindowWithoutCloseButton::Create(const long chart, const string name, const int subwin, const int x1, const int y1, const int x2, const int y2) {
...
//---
	...
	case PROGRAM_INDICATOR:
		...
		break;
	default:
		Print("CAppWindowWithoutCloseButton: invalid program type");
		return (false);
	}
...
//+------------------------------------------------------------------+
bool CAppWindowWithoutCloseButton::CreateCommon(const long chart, const string name,
		const int subwin) {
//--- save parameters
	m_chart_id = chart;
	m_name = name;
...
//--- specify object and mouse events
	if (!m_chart.EventObjectCreate() || !m_chart.EventObjectDelete() || !m_chart.EventMouseMove()) {
		Print("CAppWindowWithoutCloseButton: object events specify error");
		m_chart.Detach();
		return (false);
	}
...
//+------------------------------------------------------------------+
bool CAppWindowWithoutCloseButton::CreateExpert(const int x1, const int y1, const int x2, const int y2) {
...
//--- call method of the parent class
	if (!CWindowWithoutCloseButton::Create(m_chart.ChartId(), m_instance_id, m_subwin, x1, y1, x2, y2)) {
		Print("CAppWindowWithoutCloseButton: expert dialog create error");
		m_chart.Detach();
		return (false);
	}
//--- succeed
...
//+------------------------------------------------------------------+
bool CAppWindowWithoutCloseButton::CreateIndicator(const int x1, const int y1, const int x2, const int y2) {
...
//--- determine subwindow
	...
	if (m_subwin == -1) {
		Print("CAppWindowWithoutCloseButton: find subwindow error");
		m_chart.Detach();
		return (false);
	}
//---
...
//--- check if subwindow is not occupied by other indicators
	if (total != 1) {
		Print("CAppWindowWithoutCloseButton: subwindow busy");
...
//--- resize subwindow by dialog height
	if (!IndicatorSetInteger(INDICATOR_HEIGHT, (y2 - y1) + 1)) {
		Print("CAppWindowWithoutCloseButton: subwindow resize error");
		ChartIndicatorDelete(m_chart.ChartId(), m_subwin, ChartIndicatorName(m_chart.ChartId(), m_subwin, total - 1));
		m_chart.Detach();
		return (false);
	}
//--- indicator short name
	...
	if (!IndicatorSetString(INDICATOR_SHORTNAME, m_indicator_name)) {
		Print("CAppWindowWithoutCloseButton: shortname error");
		ChartIndicatorDelete(m_chart.ChartId(), m_subwin, ChartIndicatorName(m_chart.ChartId(), m_subwin, total - 1));
		m_chart.Detach();
		return (false);
	}
//--- set flag 
	m_panel_flag = true;
//--- call method of the parent class
	if (!CWindowWithoutCloseButton::Create(m_chart.ChartId(), m_instance_id, m_subwin, 0, 0, width, y2 - y1)) {
		Print("CAppWindowWithoutCloseButton: indicator dialog create error");
		ChartIndicatorDelete(m_chart.ChartId(), m_subwin, ChartIndicatorName(m_chart.ChartId(), m_subwin, total - 1));
		m_chart.Detach();
		return (false);
	}
...
//+------------------------------------------------------------------+
void CAppWindowWithoutCloseButton::Destroy(const int reason) {
//--- destroyed already?
...
//--- detach chart object from chart
	m_chart.Detach();
//--- call parent destroy
	CWindowWithoutCloseButton::Destroy();
//---
...
//+------------------------------------------------------------------+
void CAppWindowWithoutCloseButton::SubwinOff(void) {
	m_subwin_Yoff = m_chart.SubwindowY(m_subwin);
}
//+------------------------------------------------------------------+
//| Create the "Minimize/Maximize" button                            |
//+------------------------------------------------------------------+
bool CAppWindowWithoutCloseButton::CreateButtonMinMax(void) {
	int off = (m_panel_flag) ? 0 : 2 * CONTROLS_BORDER_WIDTH;
//--- coordinates
...
//+------------------------------------------------------------------+
void CAppWindowWithoutCloseButton::ChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam) {
...
//| Run application                                                  |
//+------------------------------------------------------------------+
bool CAppWindowWithoutCloseButton::Run(void) {
...
//--- here we begin to assign IDs to controls
	if (Id(m_subwin * CONTROLS_MAXIMUM_ID) > CONTROLS_MAXIMUM_ID) {
		Print("CAppWindowWithoutCloseButton: too many objects");
		return (false);
	}
//--- succeed
	return (true);
...
//| Stop application                                                 |
//+------------------------------------------------------------------+
void CAppWindowWithoutCloseButton::OnClickButtonClose(void) {
//--- destroy application
	Destroy();
}
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
void CAppWindowWithoutCloseButton::OnClickButtonMinMax(void) {
...
//| Resize                                                           |
//+------------------------------------------------------------------+
bool CAppWindowWithoutCloseButton::Rebound(const CRect &rect) {
...
//+------------------------------------------------------------------+
void CAppWindowWithoutCloseButton::Minimize(void) {
//--- set flag
	m_minimized = true;
//--- hide client area
...
//+------------------------------------------------------------------+
void CAppWindowWithoutCloseButton::Maximize(void) {
//--- reset flag
	m_minimized = false;
//--- show client area
...
//+------------------------------------------------------------------+
string CAppWindowWithoutCloseButton::CreateInstanceId(void) {
	return (IntegerToString(rand(), 5, '0'));
}
//+------------------------------------------------------------------+
//| Handler of the ON_APP_CLOSE external event                       |
//+------------------------------------------------------------------+
void CAppWindowWithoutCloseButton::OnAnotherApplicationClose(const long &lparam, const double &dparam, const string &sparam) {
...
//+------------------------------------------------------------------+
void CAppWindowWithoutCloseButton::IniFileSave(void) {
...
//+------------------------------------------------------------------+
void CAppWindowWithoutCloseButton::IniFileLoad(void) {
	string filename = IniFileName() + IniFileExt();
...
//+------------------------------------------------------------------+
string CAppWindowWithoutCloseButton::IniFileName(void) const {
	string name;
//---
...
//| Load data                                                        |
//+------------------------------------------------------------------+
bool CAppWindowWithoutCloseButton::Load(const int file_handle) {
	if (CWindowWithoutCloseButton::Load(file_handle)) {
		if (m_minimized) {
			m_button_minmax.Pressed(true);
			Minimize();
		} else {
...
//| Save data                                                        |
//+------------------------------------------------------------------+
bool CAppWindowWithoutCloseButton::Save(const int file_handle) {
	if (CWindowWithoutCloseButton::Save(file_handle)) {
